...
\begin{itemize}
\item El taller debe ser terminado antes de las 23:59 horas del 20 de marzo de 2022.
\item No seguir las instrucciones tiene una penalización del \textbf{20\%} de la nota final.
\item En el repositorio asignado, debe incluir tres carpetas: input (datos originales), output (datos procesados) y code (script con la respuesta del taller).
\item Debe escribir en  el siguiente Excel \textcolor{cyan}{\href{https://uniandes-my.sharepoint.com/:x:/g/personal/ef_martinezg_uniandes_edu_co/EVRAqeHLnEpEmlPTtvIApt8BXdxRDCGb3jIFLieUSAB8Yg}{group-task.xlsx}} su código, su usuario de GitHub (por favor respetar mayúsculas y minúsculas) y en la columna grupo debe escribir un número que indique el grupo al cual pertenece (si va a desarrollar el task en grupo). 
...
\item En las primeras líneas del script debe escribir su nombre, código y la versión de R sobre la que está trabajando.
\item Para este taller solo puede usar las librerías `pacman`, `rio` y `tidyverse`
\end{itemize}
